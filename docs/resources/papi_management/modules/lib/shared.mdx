...
description: Documentation for shared library functions in the 420_resource resource. These functions provide common utilities for string manipulation, math operations, table handling, and vehicle-related functionality that can be used on both client and server sides.
---
...
Used for [`libSxn.isWearingGloves`](./client#iswearinggloves) and backwards compatibility.

```lua
libSxn.armsWithoutGloves.male
```
...
```lua
libSxn.armsWithoutGloves.female
```
...
```lua
libSxn.math.round(num, decimalPlaces)
```
...
```lua
libSxn.table.mapBySubfield(tbl, subfield)
```
...
local mapped = libSxn.table.mapBySubfield(tble, 'myCategory')
...
```lua
libSxn.getVehiclePlate(vehicle)
```
...
```lua
libSxn.generateRandomPlate(pattern)
```
...
```lua
libSxn.getCardinalDirection(entity)
```
...
```lua
libSxn.array.contains(arr, value)
```
